...
class HoveringVehicle {
    ...
    constructor(properties) {
        ...
        this.vehicleNormal = new THREE.Vector3(0, 1, 0);
        this.vehicleRight = new THREE.Vector3(1, 0, 0);
        this.vehicleForward = new THREE.Vector3(1, 0, 0);
    ...
    applyMovement(delta){
        ...
        if(self.moveForward){
            self.tempVector.copy(self.vehicleForward).multiplyScalar(2.5);
        ...
        if(self.moveRight){
            self.tempVector.copy(self.vehicleRight).multiplyScalar(0.8);
            self.movementForce.add(self.tempVector);
        }
        if(self.moveLeft){
            self.tempVector.copy(self.vehicleRight).multiplyScalar(-0.8);
    ...
    initializePointsAndNeighborConstraints() {
        ...
        self.tempVector.copy(self.object3D.up).normalize().multiplyScalar(boundingSphere.radius);
        self.corners = [];
        self.corners.push(self.object3D.position.clone().sub(self.vehicleRight).sub(self.vehicleForward).sub(self.tempVector));
        self.corners.push(self.object3D.position.clone().add(self.vehicleRight).sub(self.vehicleForward).sub(self.tempVector));
        self.corners.push(self.object3D.position.clone().sub(self.vehicleRight).add(self.vehicleForward).sub(self.tempVector));
        self.corners.push(self.object3D.position.clone().add(self.vehicleRight).add(self.vehicleForward).sub(self.tempVector));
        self.corners.push(self.object3D.position.clone().sub(self.vehicleRight).sub(self.vehicleForward).add(self.tempVector));
        self.corners.push(self.object3D.position.clone().add(self.vehicleRight).sub(self.vehicleForward).add(self.tempVector));
        self.corners.push(self.object3D.position.clone().sub(self.vehicleRight).add(self.vehicleForward).add(self.tempVector));
        self.corners.push(self.object3D.position.clone().add(self.vehicleRight).add(self.vehicleForward).add(self.tempVector));

        
        self.previousCorners = [];
        self.previousCorners.push(self.corners[0].clone());
        self.previousCorners.push(self.corners[1].clone());
        self.previousCorners.push(self.corners[2].clone());
        self.previousCorners.push(self.corners[3].clone());
        self.previousCorners.push(self.corners[4].clone());
        self.previousCorners.push(self.corners[5].clone());
        self.previousCorners.push(self.corners[6].clone());
        self.previousCorners.push(self.corners[7].clone());

        const distSide = boundingSphere.radius*2;
        const distFrontBack = boundingSphere.radius*2;
        const distDiagonal = Math.sqrt(distSide * distSide + distFrontBack * distFrontBack);

        self.vehicleRight.normalize();
        self.vehicleForward.normalize();
        self.object3D.up.crossVectors(self.vehicleRight, self.vehicleForward);
        /*const constraints = [
            [0,1, distSide],
            [0,2,distFrontBack],
            [0,3,distDiagonal],
        ...
        const constraints = [];
            for(let i = 0; i<self.corners.length-1; i++){
                for(let j = i+1; j<self.corners.length; j++){
                    constraints.push([i,j,self.corners[i].distanceTo(self.corners[j])])
                }
            }
